...
new class extends Component {
    ...
    public function placeholder()
    {
        return <<<'HTML'
        <div role="status">
            <svg aria-hidden="true" class="w-8 h-8 text-gray-200 animate-spin dark:text-gray-600 fill-cyan-600" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <x-card wire:key='{{ $note->id }}'>
                        <div class="flex justify-between">
                            <div>
                                ...
                                <p class="text-xs mt-2">{{ \Illuminate\Support\Str::limit($note->body, 50) }}</p>
                            </div>
                            <div class="text-xs text-gray-500">{{ Carbon::parse($note->send_date)->format('Y-m-d') }}</div>
                        </div>
